...
def main():
    ...
    if len(extra_information) % 3 == 0:
        number_of_blcok_to_record_extra_information = len(
            extra_information) / 3
    elif len(extra_information) % 3 == 2:
        number_of_blcok_to_record_extra_information = math.floor(
            len(extra_information)/3)+1
    else:
        number_of_blcok_to_record_extra_information = math.floor(
            len(extra_information)/3)+1
    ...
    total_pixl_need_modify = number_of_pixel_should_be_modified + \
        number_of_blcok_to_record_extra_information
    ...
    print("secret data的長度(bits)：", length_of_secret,
          "bits", "(計算方式為轉換成Base64之後的資料長度)")
    print("secret data的容量大小為", length_of_secret/8, "Bytes",
          "-->", length_of_secret/8/1000, "KBytes")
